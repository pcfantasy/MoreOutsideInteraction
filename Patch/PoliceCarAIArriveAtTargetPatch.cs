...
namespace MoreOutsideInteraction.Patch
{
    ...
    public static class PoliceCarAIArriveAtTargetPatch
    {
        ...
        public static bool Prefix(ref PoliceCarAI __instance, ushort vehicleID, ref Vehicle data, ref bool __result)
        {
            if (Singleton<BuildingManager>.instance.m_buildings.m_buffer[(int)data.m_targetBuilding].m_flags.IsFlagSet(Building.Flags.Untouchable))
            {
                ...
                int num = Mathf.Min(0, (int)data.m_transferSize - __instance.m_criminalCapacity);
                ...
                if ((instance.m_buildings.m_buffer[(int)data.m_targetBuilding].m_flags & Building.Flags.IncomingOutgoing) == Building.Flags.Incoming)
                {
                    if (Loader.isRealCityRunning)
                    {
                        ...
                        z = (z > 0) ? z : -z;
                        double distance = (x + z);
                        Singleton<EconomyManager>.instance.AddPrivateIncome((int)(-num * (distance * 4f)), ItemClass.Service.PoliceDepartment, ItemClass.SubService.None, ItemClass.Level.Level3, 115);
