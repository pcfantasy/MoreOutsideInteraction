...
{
    [HarmonyPatch]
    public static class FireTruckAIArriveAtTargetPatch
    {
        public static MethodBase TargetMethod()
        {
            return typeof(FireTruckAI).GetMethod("ArriveAtTarget", BindingFlags.NonPublic | BindingFlags.Instance, null, new Type[] { typeof(ushort), typeof(Vehicle).MakeByRefType()}, null);
        }
        public static bool Prefix(ref FireTruckAI __instance, ushort vehicleID, ref Vehicle data, ref bool __result)
        {
            if (Singleton<BuildingManager>.instance.m_buildings.m_buffer[(int)data.m_targetBuilding].m_flags.IsFlagSet(Building.Flags.Untouchable))
            {
                ...
                int num = Mathf.Min(0, (int)data.m_transferSize - __instance.m_fireFightingRate);
                ...
                if (instance.m_buildings.m_buffer[(int)data.m_targetBuilding].Info.m_buildingAI is OutsideConnectionAI)
                {
                    if (Loader.isRealCityRunning)
                    {
                        ...
                        z = (z > 0) ? z : -z;
                        double distance = (x + z);
                        Singleton<EconomyManager>.instance.AddPrivateIncome((int)(-num * distance * 2f), ItemClass.Service.FireDepartment, ItemClass.SubService.None, ItemClass.Level.Level3, 115333);
